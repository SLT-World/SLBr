...
using static SLBr.Handlers.SafeBrowsingHandler;
...
namespace SLBr.Handlers
{
	public class RequestHandler : IRequestHandler
	{
        ...
        /*public long Image_Budget_Used;
		public long Image_Budget = 2 * 1024 * 1024;
		public long Stylesheet_Budget_Used;
		public long Stylesheet_Budget = 400 * 1024;
		public long Script_Budget_Used;
		public long Script_Budget = 500 * 1024;
		public long Font_Budget_Used;
        ...
        /*public long Connection_Budget_Used;
		public long Connection_Budget = 10;

        public bool CanAffordAnotherConnection()
		{
			return Connection_Budget_Used < Connection_Budget;
		}*/
        ...
        public bool IsOverBudget(ResourceType _ResourceType)
		{
            ...
            /*switch (_ResourceType) {
              case ResourceType.Image:
                return Image_Budget_Used >= Image_Budget;
              case ResourceType.Stylesheet:
                return Stylesheet_Budget_Used >= Stylesheet_Budget;
              case ResourceType.Script:
                return Script_Budget_Used >= Script_Budget;
              case ResourceType.FontResource:
                return Font_Budget_Used >= Font_Budget;
              default:
                //Not image, stylesheet, script, or font
                return false;
            }*/
            switch (_ResourceType)
            {
        ...
        public void DeductFromBudget(ResourceType _ResourceType, long DataLength)
        {
            /* Currently do not support budgeting for any of:
              ...
              ResourceType::kTextTrack:
              ResourceType.MainFrame:
              ResourceType::kImportResource:
              ResourceType.Media:
              ResourceType::kManifest:
              ResourceType::kMock::
            */
            /*switch (_ResourceType)
            {
                case ResourceType.Image:
                    Image_Budget_Used += DataLength;
                    //MessageBox.Show($"{_ResourceType} {Image_Budget_Used}:{Image_Budget}");
                    return;
                case ResourceType.Stylesheet:
                    Stylesheet_Budget_Used += DataLength;
                    //MessageBox.Show($"{_ResourceType} {Stylesheet_Budget_Used}:{Stylesheet_Budget}");
                    return;
                case ResourceType.Script:
                    Script_Budget_Used += DataLength;
                    //MessageBox.Show($"{_ResourceType} {Script_Budget_Used}:{Script_Budget}");
                    return;
                case ResourceType.FontResource:
                    Font_Budget_Used += DataLength;
                    //MessageBox.Show($"{_ResourceType} {Font_Budget_Used}:{Font_Budget}");
                    return;
                default:
                    break;
            }*/
            switch (_ResourceType)
            {
                case ResourceType.Image:
                    ...
                    //MessageBox.Show($"{_ResourceType} {Image_Budget}");
                    return;
                case ResourceType.Stylesheet:
                    Stylesheet_Budget -= DataLength;
                    //MessageBox.Show($"{_ResourceType} {Stylesheet_Budget}");
                    return;
                case ResourceType.Script:
                    Script_Budget -= DataLength;
                    //MessageBox.Show($"{_ResourceType} {Script_Budget}");
                    return;
                case ResourceType.FontResource:
                    Font_Budget -= DataLength;
                    //MessageBox.Show($"{_ResourceType} {Font_Budget}");
                    return;
                default:
                    break;
            }
        ...
        /*Image_Budget_Used = 0;
            Stylesheet_Budget_Used = 0;
            Script_Budget_Used = 0;
            Font_Budget_Used = 0;*/
            //Connection_Budget_Used = 0;
        }
				...
				if (App.Instance.GoogleSafeBrowsing)
				{
					...
					if (CheckSafe && Utils.IsHttpScheme(request.Url))
					{
						...
						if (_ThreatType == SafeBrowsingHandler.ThreatType.Malware || _ThreatType == SafeBrowsingHandler.ThreatType.Unwanted_Software)
						{
							...
							{
								if (frame.IsMain)
									chromiumWebBrowser.NewLoadHtml(App.Instance.Malware_Error, request.Url, Encoding.UTF8, 1, _ThreatType.ToString());
						...
						else if (_ThreatType == SafeBrowsingHandler.ThreatType.Social_Engineering)
						{
							...
							{
								if (frame.IsMain)
									chromiumWebBrowser.NewLoadHtml(App.Instance.Deception_Error, request.Url, Encoding.UTF8, 1, _ThreatType.ToString());
		...
		{
			chromiumWebBrowser.ExecuteScriptAsync(@"var style = document.createElement('style');style.textContent = `::-webkit-scrollbar {width:15px;border-radius:10px;border:5px solid transparent;background-clip:content-box;background-color: whitesmoke;}
::-webkit-scrollbar-thumb {height:56px;border-radius:10px;border:5px solid transparent;background-clip:content-box;background-color: gainsboro;transition:background-color 0.5s;}
::-webkit-scrollbar-thumb:hover{background-color:gray;transition:background-color 0.5s;}
::-webkit-scrollbar-corner{background-color:transparent;}`;document.head.append(style);");
